...
import 'bottom_funtionalality.dart';

class PostOfSingleImage extends StatelessWidget {
  const PostOfSingleImage({
    Key key,
    this.postmodel,
    this.index,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.only(top: 16, left: 12, bottom: 16, right: 12),
            ...
            child: ClipRRect(
              ...
              child: SizedBox(
                ...
                child: GestureDetector(
                  onTap: () => Get.to(
                    () => FullImagePreview(
                        imageUrl: NETWORK_IMAGE_URL + postmodel.imageList[0],
                        blurHash: postmodel.blurHash[0]),
                  ),
                  child: BlurHash(
                    hash: postmodel.blurHash[0],
                    image: NETWORK_IMAGE_URL + postmodel.imageList[0],
                    imageFit: BoxFit.fitHeight,
                    decodingHeight: (SizeConfig.heightMultiplier * 28).round(),
                    decodingWidth: (SizeConfig.widthMultiplier * 90).round(),
